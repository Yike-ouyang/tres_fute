...
# It regenerates rl_env_2/train_shaped.py from the marimo notebook (single
# source of truth), launches the three variants (min_zone, pbrs, variance)
# in parallel as detached processes, and starts TensorBoard bound to
# 127.0.0.1 (reachable through an SSH tunnel).
#
# Prerequisites on the server (once):
#   * repo cloned                             -> backend/runs/ must exist
#   * models copied from the local machine    -> see "sync models" below
#   * scripts/run_shaped_remote.sh install    (venv + dependencies)
#
# Local -> server model copy (run from the LOCAL repo root):
#   rsync -avR \
#     backend/runs/essai_01/best_model.zip \
#     backend/runs/solo_*/score_delta_solo/best_model.zip \
...
#   scripts/run_shaped_remote.sh status [--output-dir DIR]
#   scripts/run_shaped_remote.sh logs   [--output-dir DIR]
#   scripts/run_shaped_remote.sh stop   [--output-dir DIR] [--all]
#   scripts/run_shaped_remote.sh tensorboard [--host H] [--port P] [--output-dir DIR]
#   scripts/run_shaped_remote.sh replays [--seed N] [--output-dir DIR]
#   scripts/run_shaped_remote.sh help
...
VENV="$BACKEND/.venv"
PY="$VENV/bin/python"
NOTEBOOK="$BACKEND/rl_env_2/shaped_reward_training.py"
TRAIN_SCRIPT="$BACKEND/rl_env_2/train_shaped.py"
RUNS="$BACKEND/runs"
...
cmd_install() {
    ...
    echo "Note: requirements.txt est un freeze complet avec CUDA/NVIDIA; on ne l'utilise pas ici."
}
...
cmd_replays() {
    ...
    for mode in min_zone pbrs variance; do
        ...
        "$PY" "$BACKEND/rl_env_2/watch_best_model.py" --model "$best" --seed "$REPLAY_SEED"
    done
