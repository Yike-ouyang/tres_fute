...
def _runs_root() -> Path:
    return _BACKEND / "runs"


def _run_label(model_path: Path) -> str:
    """Informative run label, e.g. ``solo_<ts>/score_delta_solo`` or ``essai_01``."""
    model_dir = model_path.parent
    root = _runs_root()
    try:
        return str(model_dir.relative_to(root))
    except ValueError:
...
def _default_out_dir(model_path: Path) -> Path:
    ...
    The replay API scans ``runs/*/replays/index.json`` (one level). A solo model
    lives in ``runs/<ts>/score_delta_solo``, so we target the run directory
    directly under ``runs`` (here ``runs/<ts>``).
    """
    model_dir = model_path.parent
    root = _runs_root()
    try:
...
def main(argv=None):
    ...
    try:
        ...
        raise SystemExit('Lancer depuis backend, avec ce script dans rl_env_2/.\n' + str(exc)) from exc
    ...
    checkpoint = Path(args.checkpoint or _checkpoint_from(meta, config) or DEFAULT_CHECKPOINT).resolve()
    if not checkpoint.is_file():
        raise FileNotFoundError(
            f'Checkpoint introuvable: {checkpoint}. Précisez --checkpoint (aucun fallback heuristique).'
        )
