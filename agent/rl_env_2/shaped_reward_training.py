...
def _(Path, sys):
    ...
    _candidates = (here, *here.parents)
    # agent_dir = dossier qui contient rl_env_2 (le notebook y vit).
    agent_dir = next(
        (p for p in _candidates if (p / "rl_env_2" / "shaped_reward_training.py").is_file()),
        None,
    )
    if agent_dir is None:
        agent_dir = next(
            (p / "agent" for p in _candidates
             if (p / "agent" / "rl_env_2" / "shaped_reward_training.py").is_file()),
            None,
        )
    if agent_dir is None:
        raise RuntimeError(f"Impossible de trouver agent/rl_env_2 depuis {here}")
    # backend/ (game_engine, simulation) est un voisin de agent/.
    backend_dir = agent_dir.parent / "backend"
    if not (backend_dir / "game_engine").is_dir():
        raise RuntimeError(f"backend/game_engine introuvable à côté de {agent_dir}")
    for root in (agent_dir, backend_dir):
...
@app.cell
def _(agent_dir, datetime, timezone):
    CONFIG = {
        # Env
...
def _(agent_dir):
    candidates = sorted(
        (agent_dir / "runs").glob("solo_*/score_delta_solo/best_model.zip"),
...
def _(textwrap):
    ...
    _agent_dir = _here.parents[1]                  # agent/ (rl_env, rl_env_2)
...
@app.cell
def _(TRAINING_SCRIPT, agent_dir):
    SCRIPT_PATH = agent_dir / "rl_env_2" / "train_shaped.py"
